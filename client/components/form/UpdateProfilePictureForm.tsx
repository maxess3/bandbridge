		...
		<div className="space-y-6 min-h-46">
			{file ? (
				...
				<div>
					<div
						{...getRootProps()}
						className={`min-h-56 flex items-center justify-center border-2 border-dashed rounded-lg p-6 text-center cursor-pointer hover:border-border ${
							isDragActive
								? "!border-solid border-link bg-secondary"
								: "border-border-light"
						}`}
					>
						<input {...getInputProps()} />
						<div className="flex flex-col items-center gap-6 font-medium">
							<MdOutlineFileUpload className="text-4xl text-foreground/50" />
							<p className="text-foreground/90">
								Glissez-déposez une image, ou
								{` `}
								<span className="font-bold underline text-link">
									cliquez ici
								</span>
								{` `}
