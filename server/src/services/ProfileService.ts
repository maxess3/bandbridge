...
import { Prisma, Platform, MusicGenre } from "../generated/client";
...
export class ProfileService {
   ...
   * For pagination mode, limit is fixed at 24 items per page.
   *
   * @param query - Search query string
   * @param options - Search options (mode, limit, page). Limit is fixed at 24 for pagination mode.
  ...
  /**
   * Retrieves all profiles with pagination support.
   * Similar to searchProfiles but without search filtering.
  ...
  static async getAllProfiles(
    ...
    // Fetch profiles with classic pagination (no where clause - all profiles)
    const profiles = await prisma.profile.findMany({
      skip,
      take: limitNumber,
      orderBy: { id: "asc" },
