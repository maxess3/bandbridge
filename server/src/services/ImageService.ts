...
export class ImageService {
	...
	/**
	 * Uploads a band picture in multiple sizes and updates the band.
	 *
	 * @param bandId - The band ID
	 * @param file - The uploaded file from Multer
	 * @returns The band picture key (medium size)
	 *
	 * @throws {NotFoundError} If band is not found
	 */
	static async uploadBandPicture(
		bandId: string,
		file: Express.Multer.File
	): Promise<string> {
		const uuid = uuidv4();
		...
		// Get current band to check for existing picture
		...
		// Delete old images if they exist
		if (currentBand.profilePictureKey) {
			await this.deleteImageVariants(currentBand.profilePictureKey);
		}
		...
		const uploadPromises = Object.entries(IMAGE_SIZES).map(
			async ([size, width]) => {
				...
				const key = `band-pictures/${bandId}/${uuid}-${size}.webp`;
		...
		// Update band with main image key (medium size)
		const mainImageKey = uploadedImages.find(
			(img) => img.size === "medium"
		)?.key;
		if (!mainImageKey) {
			throw new Error("Failed to upload band picture");
		}
